  ...
  try {
    ...
    if (update.message && update.message.text) {
      ...
      if (text === "/start" || text.startsWith("/start ")) {
        const archiveUrl = url.origin;

        const welcomeText =
          `<b>🤖 Bot Arşivi'ne Hoş Geldin!</b>\n\n` +
          `Telegram'ın en kullanışlı botlarını tek yerde keşfet.\n\n` +
          `📥 İndirici botlar\n` +
          `🤖 Yapay zeka botları\n` +
          `📈 Finans &amp; kripto botları\n` +
          `🎵 Müzik &amp; ses botları\n` +
          `🛠 Araçlar &amp; dosya botları\n` +
          `🎮 Oyun ve eğlence botları\n\n` +
          `<b>Botunu bul, incele ve tek dokunuşla Telegram'da başlat.</b>`;

        const keyboard = {
          inline_keyboard: [
            [
              { text: "🗂 Arşive Git", web_app: { url: archiveUrl } }
            ],
            [
              { text: "💬 Feedback", url: "https://t.me/baygoktas" }
            ]
          ]
        };
